...
def star(length: int, depth: int):
  ...
  for _ in range(S):
    ...
    )  # ensures that different depths are colored differently so that it is visually clear
    pen.forward(length)
    star(
      length // N, depth -
      1)  # a recursive call with length divided by N and depth decreased by 1
    pen.right(180 - angle)
